...
import { Music, LayoutDashboard, Users, FolderKanban, Bell, DollarSign,
  Award, Calendar, Upload, MessageSquare, LogOut, ChevronLeft, ChevronRight, Settings } from 'lucide-react';
...
const PRODUCER_NAV = [
  ...
  { to: '/dashboard/producer/messages',   label: 'Messages',    icon: MessageSquare },
...
const DashboardSidebar = () => {
  ...
  return (
    <aside className={`${collapsed ? 'w-16' : 'w-60'} bg-dark-950 border-r border-purple-900/30 min-h-screen flex flex-col transition-all duration-300 flex-shrink-0`}>
      ...
      {/* Bottom */}
      <div className={`px-2 pb-4 pt-2 border-t border-purple-900/30 space-y-1 ${collapsed ? '' : ''}`}>
        {!collapsed && (
