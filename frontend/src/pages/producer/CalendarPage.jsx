...
import { Calendar, ChevronLeft, ChevronRight, Clock, FolderKanban } from 'lucide-react';
...
export default function ProducerCalendarPage() {
  ...
  return (
    <DashboardLayout title="Calendar" subtitle="Your task deadlines and schedule">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card-dark p-5">
          ...
          {upcoming.length === 0 ? (
            ...
            <div className="space-y-3">
              {upcoming.map(t => {
                ...
                return (
                  <div key={t.id} className="p-3 rounded-xl bg-dark-800/80 border border-purple-900/20">
                    <div className="flex items-center gap-2 mb-1">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${statusColor(t.status)}`} />
                      <p className="text-white text-sm font-display font-semibold truncate">{t.title}</p>
                    </div>
                    <p className="text-purple-400 text-xs">Client: {t.client_name}</p>
          ...
          {tasks.length > 0 && (
            <>
              <h3 className="font-display font-bold text-white mt-5 mb-3 text-xs uppercase tracking-widest text-purple-500">All Tasks</h3>
              <div className="space-y-1.5">
                {tasks.map(t => (
                  <div key={t.id} className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${statusColor(t.status)}`} />
                    <p className="text-purple-300 text-xs truncate">{t.title}</p>
                  </div>
                ))}
              </div>
            </>
